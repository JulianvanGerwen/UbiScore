#!/usr/bin/env python3
# Derived from https://github.com/evocellnet/ptm_hotspots
...
def permutated_dataframe(letter_alignment, phosp_alignment, column_names, indexes, aa_letters="STY"):
    """
    Compute a permutated alignment column by running a rolled window permutation on the 
    modified amino acids specified in aa_letters.

    Parameters:
      letter_alignment: DataFrame with the amino acid letter for each protein.
      phosp_alignment: DataFrame with numerical phosphorylation data.
      column_names: List of protein identifiers (columns) in the DataFrames.
      indexes: List of row indexes.
      aa_letters: A string (or list) of target amino acids to process (default "STY").

    Returns:
      A list representing one permutation of the background window.
    """

    # Initialize the DataFrame to hold permutated values
    permutated_alignment = pd.DataFrame(
        0,
        index=indexes[2:],  # same as in your original function
        columns=column_names,
    )
    
    # Loop through each protein (column)
    for protein_id in column_names:
        # Get indices where the phosphorylation value is >= 1
        phosp_rows = phosp_alignment.index[phosp_alignment[protein_id] >= 1].tolist()
        
        # Process each amino acid in aa_letters
        for letter in aa_letters:
            # Find rows where the letter_alignment equals the target amino acid
            rows_with_letter = letter_alignment.index[letter_alignment[protein_id] == letter].tolist()
            
            # Count how many of the phosphorylated rows are in the rows_with_letter list
            count_letter = sum(1 for i in phosp_rows if i in rows_with_letter)
            
            # Generate a random histogram permutation for this letter group.
            random_hist = make_random_histogram(count_letter, len(rows_with_letter))
            
            # Assign the permutated values to the appropriate rows
            for value, row in zip(random_hist, rows_with_letter):
                permutated_alignment.at[row, protein_id] = value

    # Sum across proteins (rows) and run the count_window_for_list function on the sums
...
def getHotspotSitesInFile(alignment_path, ptms, repeat, aa_letters = "STY"):
    # Starting dataframes
...
def find_hotspot_instances(hotspot_sites, hotspot_definitions):
    ...
    # This will collapse cases where one protein has multiple domains
    # start and end will be the beginning of the first domain and the end of the last
    out = h_inst \
...
if __name__ == '__main__':
    ...
    parser.add_argument('--printSites',
                        ...
                        default=False)
    # New argument for target amino acids
    parser.add_argument('--aa_letters',
                        nargs='?',
                        default="STY",
                        action="store",
                        metavar="STRING",
                        dest="AA_LETTERS",
                        help='target amino acids to process (default: "STY")')
    #New argument for using raw p-values
    parser.add_argument('--useRawPvals',
                        dest="USERAWPVALS",
                        action="store_true",
                        help='use raw p-values instead of adjusted',
                        default=False)
    ...
    AA_LETTERS = results.AA_LETTERS
    USERAWPVALS = results.USERAWPVALS
    ...
    for filename in alignmentFiles:
        ...
        expanded_dataframe = getHotspotSitesInFile(alignment_path, ptms, ITER, aa_letters = AA_LETTERS)
    ...
    if USERAWPVALS:
        h_sites["hotspot"] = (h_sites.pvals <= THRESHOLD) & \
